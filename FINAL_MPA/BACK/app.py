...
# Adiciona um usuário administrador pré-definido
def add_admin_user():
    ...
    cursor.execute("SELECT COUNT(*) FROM users WHERE login = ?", (admin_login,))
    exists = cursor.fetchone()[0]
    
    if not exists:
        cursor.execute('''
            INSERT INTO users (login, senha, administrador)
            VALUES (?, ?, ?)
        ''', (admin_login, admin_senha, 1))
        conn.commit()
        print("Usuário administrador adicionado com sucesso!")
    else:
        print("Usuário administrador já existe.")
    
    conn.close()

# Chama a função para adicionar o usuário administrador
add_admin_user()
